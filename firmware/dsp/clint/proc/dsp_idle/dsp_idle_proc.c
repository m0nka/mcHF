...
ushort ui_driver_get_buffer(uchar *buffer)
{
	ulong k;
	...
	// Left part of screen
	for(k = 0; k < 512;k++)
		buffer[k +   0] = (uchar)*(sd_FFT_DspData + k + 512);

	// Right part of screen
	for(k = 0; k < 512;k++)
		buffer[k + 512] = (uchar)*(sd_FFT_DspData + k + 0);

	// -------------------------------------------
	// -------------------------------------------
	// -------------------------------------------
	// Debug only - monitor FFT data
	if((buffer[400] == buffer[401]) &&\
	   (buffer[401] == buffer[402]) &&\
	   (buffer[402] == buffer[403]) &&\
	   (buffer[403] == buffer[404]))
	{
	...
	// Good data:
	// [m4]  cdc: 0x0, twt: 6, sf: 4, dbs: 0x1f, dof: 0x0, agr: 0x1
	#if 0
	printf("cdc: 0x%x, twt: %d, sf: %d, dbs: 0x%x, dof: 0x%x, agr: 0x%x\r\n", 	(ulong)ads.codec_gain_calc,
																				ts.fft_window_type,
																				ts.scope_filter,
...
static void UiDriverReDrawSpectrumDisplay(void)
{
	...
	q15_t   	sd_FFT_TempData[FFT_IQ_BUFF_LEN1];							// Craps out if moved as static ??????
																			// ToDo: check!!!
...
//!		return;
	...
	#if 0
	//#ifndef USE_FFT_BACK_BUFFER
	static ulong sk_ipped = 0;
	static ulong sk_total = 0;
	static ulong sk_acu_a = 0;
	static ulong sk_acu_b = 0;
	static ulong sk_acu_c = 0;
	static ulong sk_cnt = 0;
	sk_total++;
	#endif
	...
	//#ifndef USE_FFT_BACK_BUFFER
	#if 0
	sk_acu_a += sk_ipped;		// super loops calls while processing(CPU background load)
	sk_acu_b += sd.samp_ptr_a;	// audio driver lost IQ samples while this function does FFT
	sk_acu_c += sk_total;		// total super loops calls (CPU IDLE capacity)
	...
	if(sk_cnt == 300)
	{
		...
		//printf("------------------\r\n");
		//printf("aver cpu calls: %d\r\n", sk_acu_a);
		//printf("totl cpu calls: %d\r\n", sk_acu_c);
		//printf("aver samp lost: %d\r\n", sk_acu_b);
...
//*----------------------------------------------------------------------------
//* Function Name       : dsp_idle_proc_ptt_off
//* Object              : PTT button release handling
//* Input Parameters    :
//* Output Parameters   :
//* Functions called    :
//*----------------------------------------------------------------------------
static void dsp_idle_proc_ptt_off(void)
{
	static ulong ptt_break = 0;
...
void ui_driver_toggle_tx()
{
	static uchar curr_trx_mode = TRX_MODE_RX;

	// Is there a change ?
	if(curr_trx_mode == ts.txrx_mode)
		return;
	...
	// Save
	curr_trx_mode = ts.txrx_mode;
}

void dsp_idle_proc_change_demod_mode(uchar mode, uchar kmode)
{
	ts.dmod_mode = mode;

	// Temp shift nco - need proper handling !
	if((df.nco_freq == 0)&&(ts.dmod_mode == DEMOD_AM)) // FM as well ?
	{
		df.nco_freq = 12000;
	}
...
void dsp_idle_proc_change_agc_mode(uchar mode, int gain)
{
	ts.agc_mode = mode;
...
void dsp_idle_proc_change_filter(uchar id)
{
	ts.filter_id = id;
}
...
//	UiLcdHy28_PrintText(((POS_SM_IND_X + 18) + 140),(POS_SM_IND_Y + 59),"PROT",Black,Black,4);
	...
	//
	if(ts.power_level ==PA_LEVEL_5W)	{	// PA_LEVEL_FULL
