...
// If we want to keep on collecting data
// while background process does FFT
//#define USE_FFT_BACK_BUFFER
...
typedef struct SpectrumDisplay
{
	...
	// Samples buffer
	//
	float32_t	FFT_Samples[FFT_IQ_BUFF_LEN1*2]; 	// Front buffer

	#ifdef USE_FFT_BACK_BUFFER
	float32_t	FFT_SamplesA[FFT_IQ_BUFF_LEN1*2];	// Back buffer
	#endif
