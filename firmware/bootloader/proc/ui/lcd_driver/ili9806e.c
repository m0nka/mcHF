...
//**---------------------------------------------------------------------------------**
//**                                                                                 **
//**  File name:		ili9806e.c                                                   **
//**  Description:  	ILI9806E panel init, ported from the radio firmware         **
...
// 0x03 = 180 deg scan flip: the bootloader software-rotated drawing is
// upside down relative to the emWin mapping in the radio firmware, so the
// panel scan is flipped here instead - the radio firmware re-inits the
// panel with 0x00 at its own LCD bring up. Written twice below, so unlike
// the old randomly-lost write this applies reliably (LP 16/0 link fix)
#define ILI9806E_MADCTL			0x03
