...
uchar pll_assignment[8];
uint64_t clk_freq[8] = {SI5351_XTAL_FREQ, SI5351_XTAL_FREQ};
uint64_t plla_freq;
uint64_t pllb_freq;
uint32_t xtal_freq[2];
bool clk_first_set[8];
int32_t ref_correction[2];
uint8_t clkin_div = SI5351_CLKIN_DIV_1;
uint8_t i2c_bus_addr = SI5351_BUS_BASE_ADDR;

enum si5351_pll_input plla_ref_osc = SI5351_PLL_INPUT_XO;
enum si5351_pll_input pllb_ref_osc = SI5351_PLL_INPUT_XO;
...
bool Si5351_init(uint8_t xtal_load_c, uint32_t xo_freq, int32_t corr)
{
	...
	if(reg_val == 0)
	{
		...
		Si5351_si5351_write(SI5351_CRYSTAL_LOAD, (xtal_load_c & SI5351_CRYSTAL_LOAD_MASK) | 0b00010010);
...
uint8_t Si5351_si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
	//Wire.beginTransmission(i2c_bus_addr);
	//Wire.write(addr);
	...
	//printf("write bulk %02x: %02x\r\n", addr, bytes);

	int res= BSP_I2C2_WriteReg(i2c_bus_addr, addr, data, bytes);
	if(res)
	{
		printf("Si5351a_WriteRegister err %d\r\n", res);
		return res;
	}
...
uint8_t Si5351_si5351_write(uint8_t addr, uint8_t data)
{
	//Wire.beginTransmission(i2c_bus_addr);
	//Wire.write(addr);
	//Wire.write(data);
	...
	int res= BSP_I2C2_WriteReg(i2c_bus_addr, addr, &data, 1);
	if(res)
	{
		printf("Si5351a_WriteRegister err %d\r\n", res);
		return res;
...
uint8_t Si5351_si5351_read(uint8_t addr)
{
	...
	int res= BSP_I2C2_ReadReg(i2c_bus_addr, addr, &reg_val, 1);
	if(res)
	{
		printf("Si5351a_WriteRegister err %d\r\n", res);
		return 0;
