...
static const GUI_WIDGET_CREATE_INFO _aDialog[] = 
{
	...
	//
	{ BUTTON_CreateIndirect, 	"Update Firmware",		ID_BUTTON_UI_RESET,		40, 	40, 	120, 	45, 	0, 		0x0, 	0 },
...
static void Startup(WM_HWIN hWin, uint16_t xpos, uint16_t ypos)
{
	...
	hLdialogE = GUI_CreateDialogBox(p_widget, GUI_COUNTOF(_aDialog), _cbDialog, hWin, xpos, ypos);

	free(p_widget);
	return;

use_const_decl:
	hLdialogE = GUI_CreateDialogBox(_aDialog, GUI_COUNTOF(_aDialog), _cbDialog, hWin, xpos, ypos);
}

static void KillLora(void)
{
	//printf("kill menu\r\n");
	GUI_EndDialog(hLdialogE, 0);
}
